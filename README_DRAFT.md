...
repositories, `public_repo` may be sufficient. The setup action also rewrites
the filenames of the `collect` and `publish` workflows - so it will need
`workflows: write` permission as well.

For a token that follows the principle of least privilege more strongly,
you are advised to use a _fine-grained personal acces token_ instead:

| PAT (Classic) | PAT (Fine-Grained) | Permissions
| --- | --- | ---|
|
