...
`config.yaml` is the active configuration for this repository. It is user-owned after setup: collection and publication runs read it, but do not silently rewrite it.
...
- `collect` also uploads a `reponomics-collect-provenance` artifact after successful collection. That artifact records the collected repository revision, accepted action tag, accepted action commit SHA, and current output-generation settings so the always-present publish workflow can no-op cleanly when output generation is disabled.
