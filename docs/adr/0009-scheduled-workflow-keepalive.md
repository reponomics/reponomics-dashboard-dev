...
- Status: Proposed
- Date: 2026-05-27
- Complements: [ADR 0008](0008-incident-sentinel-artifact-preservation.md)
...
This creates a separate operational risk from collection failure. `incident-sentinel` can preserve the latest unexpired `dashboard-data` artifact when collection fails, but it cannot run if GitHub silently disables scheduled workflow triggers.
...
- The workflow does not replace incident-sentinel; keepalive is scheduler liveness, while incident-sentinel is artifact preservation during known collection outages.
