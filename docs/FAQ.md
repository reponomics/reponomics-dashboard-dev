...
In `strong` and `casual`, retained artifacts and hosted dashboard data objects are encrypted before they are stored or published. The hosted dashboard decrypts the summary after unlock and decrypts per-repository chunks as repos are selected for display.
