...
`reponomics-dashboard` is the template repository users create their own dashboard repositories from. `reponomics-dashboard-dev` is the source repository that builds and tests that generated template. `reponomics-dashboard-action` is the versioned runtime used by generated dashboard workflows; it owns collection, artifact restore/upload, schema migration, encryption, README rendering, HTML dashboard rendering, CSV export packaging, key rotation, and release notices.
...
See [Secure Dashboard Key Generation](SECURE_DASHBOARD_KEY.md).
...
Collaborators may not be able to read existing secret values directly, but if they can update repository secrets, run workflows, or affect trusted workflow behavior, they can disrupt encrypted state, replace dashboard keys, interfere with rotation/reset flows, or cause data loss. Branch rulesets can protect branches, but they are not a clean data-access boundary.
